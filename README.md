...
개발 기록 가운데 **같은 데이터셋과 같은 평가 기준으로 반복 측정한 구간만** 선으로 연결했습니다. 초기 Python 버전은 전체 필드 통과율을, C# 재작성 이후는 `tests/pic` 평균 인식 지연을 보여줍니다.

![no-kalbak 버전별 OCR 정확도와 인식 지연 변화](docs/benchmarks/performance-history.svg)

- **Python 2차 버전:** 153장 평가셋에서 이름·레어리티·부위·등급·품질·4개 능력치가 모두 맞아야 통과하는 `c` 기준이 2.00%에서 96.08%(147/153)로 상승했습니다.
- **C# 버전:** 같은 `tests/pic` 20장의 평균 인식 지연이 853ms에서 315ms로 63.1% 줄었습니다. 244ms 고속 경로 이후 실사용 정확도 회귀가 발견되어 정확도 복원 변경을 반영한 315ms를 해당 구간의 최종 기록으로 사용했습니다.
- **1.0.1 현재 기준선:** 외부 143장 데이터셋에서 핵심 필드 143/143, 능력치 값 176/176이었고 평균 459ms, P95 548ms였습니다. 파일 입력 기반 측정이므로 실제 화면 캡처, 커서 대기, WPF 표시 시간은 포함하지 않습니다.

초기 Python 1차 버전은 정량 평가 기록이 없어 0점으로 표시하지 않았습니다. 세 구간은 데이터셋과 평가 규칙이 서로 달라 수치를 하나의 순위처럼 직접 비교할 수 없습니다. [그래프 원자료](docs/benchmarks/performance-history.json), [재생성 스크립트](tools/render-performance-history.py), [벤치마크 실행 방법](docs/BENCHMARKS.md)을 함께 공개합니다.
...
- 자동 테스트 134개, 전체 143장 데이터셋의 핵심 필드 143/143과 능력치 값 176/176, 별도 폴더 배포 스모크 테스트를 확인했습니다. [공개 전 검토 기록](docs/REVIEW.md)
...
- 실제 라이브 실패 캡처 22장을 라벨링해 최종 정밀 경로 22/22를 확인하고, 실패 캡처를 보존하는 회귀 분석 흐름을 마련했습니다.
...
- 실제 게임 화면의 UI 혼입과 동적 툴팁 위치 때문에 깨끗한 평가셋 성능이 실사용으로 이어지지 않는 한계를 확인했습니다.

### 2026-04 · Python 초기 프로토타입

- PyQt5, Tesseract, Neople Open API와 SQLite를 이용해 장비 이름을 읽고 비교하는 첫 동작 버전을 만들었습니다.
- 커서 오른쪽의 고정 영역과 OCR 첫 줄에 의존했으며, 이 단계에는 정량 정확도·속도 기록이 남아 있지 않습니다.
